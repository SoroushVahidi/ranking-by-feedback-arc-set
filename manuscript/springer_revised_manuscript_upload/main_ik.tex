...
\item \textbf{Phase~C (prior lineage):} topological score extraction and optional refinement (adjacent-swap then order-preserving ternary) \citet{VK25}.
\end{enumerate}
...
$\mathrm{acyclicFlag}\leftarrow(\mathrm{kept}\ \mathrm{is\ a\ DAG})$\;
...
\ForEach{excluded arc $e=(u,v)$ in that order}{
  ...
  \If{$v\not\rightarrow^\star u$ in current kept digraph}{$\mathrm{kept}[e]\leftarrow\mathrm{true}$}
}
\tcp*{Prop.~\ref{prop:reachability_addback} is claimed only when $\mathrm{acyclicFlag}$ held after Phase~A}
\If{min-cut repair enabled \textbf{and} $\mathrm{kept}$ is a DAG}{apply Alg.~\ref{alg:mincut_exchange} under remaining budget}\;
\tcp*{if Phase~A exited cyclic, skip min-cut: Prop.~\ref{prop:mincut} assumes a DAG}
...
After Phase~A, excluded arcs are scanned once in stable descending original weight. Arc $(u,v)$ is reinserted iff there is no directed path $v\rightarrow^\star u$ in the current kept digraph. When Phase~A left a DAG, this is the exact cycle-safety test of \citet{CI03,VK25} and Prop.~\ref{prop:reachability_addback} applies; after a premature cyclic Phase~A exit the same scan is still attempted under the remaining budget, but the proposition's DAG hypotheses need not hold. A fixed-topological-position proxy (accept iff $\mathrm{pos}(u)<\mathrm{pos}(v)$ for one fixed topological order) is weaker and is retained only as an ablation baseline (INS multipass variants).
...
\midrule
$T$ (\texttt{time\_limit\_sec}) & Global wall-clock budget & Canonical non-GNN: $1800$\,s; shared by Phases~A--C; early exit may leave cycles \\
$\tau$ (\texttt{zero\_tol}) & Residual kill threshold & Default $10^{-15}$; sensitivity Z12/Z15/Z18 in \S\ref{subsec:sensitivity} \\
DFS / Kahn order & Deterministic traversal & Vertices $1..n$; stable edge order; C0/C1 cycle-selection in \S\ref{subsec:sensitivity} \\
...
Min-cut budgets K20/K50/K100 & Optional repair caps & Candidate attempt caps in \S\ref{subsec:sensitivity}; secondary \\
...
\bottomrule
\end{tabular}
\caption{Consolidated parameters and termination behavior. Canonical \textsc{OURS-Reach} uses exact reachability add-back, R2 refinement (adjacent-swap then ternary), and min-cut off.}
...
Exact reachability add-back sorts the $m$ candidate arcs once ($O(m\log m)$) and, in the dense implementation used for suite graphs with $n\le 4000$, maintains an $n\times n$ reachability matrix with an $O(n^{2})$ incremental update per accepted insertion (at most $m$ insertions), i.e.\ $O(m\log m+mn^{2})$ in the worst case; a sparse BFS/DFS fallback is used only for larger $n$. Optional min-cut repair adds budgeted directed min-cut computations. Phase~C is time-bounded and may use up to $O(n^2)$ pairwise structures for ratio refinement. Empirical timing is reported in \S\ref{subsec:runtime_coverage}--\ref{subsec:scalability}.
...
\paragraph{Methods.}
Baselines evaluated in the archived suite protocol are ten classical methods (\textsc{SpringRank}, \textsc{SyncRank}, \textsc{SerialRank}, \textsc{BTL}, \textsc{DavidScore}, eigenvector centrality, \textsc{PageRank}, \textsc{RankCentrality}, \textsc{SVD-RS}, \textsc{SVD-NRS}) and two GNNRank backbones (\textsc{DIGRAC}, \textsc{ib}) under fixed archived configurations (not post-hoc best-in-suite oracles).
Principal Tables~\ref{tab:pairwise_quality_simple}--\ref{tab:runtime_wtl} report a \emph{prespecified} principal subset of eight classical methods---\textsc{SpringRank}, \textsc{SyncRank}, \textsc{SerialRank}, \textsc{BTL}, \textsc{DavidScore}, \textsc{PageRank}, \textsc{RankCentrality}, \textsc{SVD-NRS}---plus the two GNN backbones.
\textsc{SVD-RS} and eigenvector centrality were evaluated in the same suite export but omitted from these principal tables as near-duplicate spectral companions of \textsc{SVD-NRS} and \textsc{PageRank}, respectively (selection fixed before headline regeneration; not a post-hoc cull by effect size).
An inherited GNNRank implementation bug in \textsc{RankCentrality} (normalized transition matrix overwritten before the eigenvector step) was corrected to the Negahban--Oh--Shah construction before regenerating that baseline's rows; other baselines were left unchanged.
The \emph{canonical} method reported in the principal quality/runtime tables is \textsc{OURS-Reach}: Phase~A + exact reachability-aware reinsertion + Phase~C refinement (adjacent-swap then order-preserving ternary; \S\ref{sec:framework}), \emph{without} the optional min-cut exchange.
...
RankCentrality uses the corrected Negahban--Oh--Shah implementation (\S\ref{subsec:exp_setup}).}
...
Corrected RankCentrality is stronger on this metric (as are BTL and PageRank).}
...
On \texttt{upset\_naive}, the same directional pattern holds (e.g., $46/0/31$ vs SpringRank; $76/0/1$ vs BTL; $60/0/0$ vs \textsc{ib}; $47/0/30$ vs corrected RankCentrality).
On \texttt{upset\_ratio}, BTL remains clearly stronger ($3/0/74$), PageRank also wins more often ($14/0/63$), and corrected RankCentrality is stronger still ($9/0/68$); SpringRank is competitive ($42/0/35$).
...
\textsc{OURS-Reach} is slower than most lightweight classical methods (comparable to SyncRank and corrected RankCentrality) and faster than archived trained-GNN end-to-end runs.}
...
Under a tightened Phase~A wall-clock budget of $600$\,s, Phase~A-only has single-invocation algorithm wall time $\approx 600.5$\,s (Phase~A $\approx 600.0$\,s hitting its budget, plus refinement $\approx 0.5$\,s; harness wall time, including data loading and metric computation, $\approx 612.6$\,s). Reachability and reachability+refinement configurations also have Phase~A hit the same $600$\,s budget: true single-invocation algorithm wall time is $\approx 602.3$\,s (add-back $\approx 1.8$\,s and refinement $\approx 0.5$\,s beyond Phase~A). The per-run harness timer additionally records $\approx 1214.8$/$1214.6$\,s for these two configurations because it also executes a diagnostic Phase-A-only rerun (used only to compute the permutation-distance sensitivity statistic of \S\ref{subsec:sensitivity}) inside the same timed window; this second pass is not part of a single \textsc{OURS-Reach} invocation and is excluded from the algorithm wall time above. Enabling optional min-cut hits a hard wall-clock timeout at $\approx 1800.1$\,s without a finished ranking.
...
On non-Finance ablation graphs ($n\le 602$), \textsc{OURS-Reach} single-invocation algorithm wall time is roughly $0.01$--$0.83$\,s (median $\approx 0.38$\,s); enabling optional min-cut adds about $3$--$4$\,s median.
...
A2$\to$A5 and A4$\to$A6 report \emph{structural} removed-weight improvement from the min-cut operator on the retained graph; Phase~C refinement does not alter kept arcs, so these FAS-weight claims do not depend on score-refinement order.}
\label{tab:ablation_primary}
\end{table}

Figure~\ref{fig:ablation} shows unpaired matched-support median trajectories for the legacy and canonical stage progressions (analogous to Table~\ref{tab:reviewer1_stage_ablation}); paired tests above are authoritative. Optional min-cut configs A5/A6 are omitted from the figure because they are secondary structural operators with different support/purpose; their paired FAS-weight tests appear in Table~\ref{tab:ablation_primary}.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{fig_structural_ablation.pdf}
\caption{Matched-support unpaired medians for (a) legacy A0$\to$A1$\to$A3 on common $n=33$ and (b) canonical A0$\to$A2$\to$A4 on common $n=77$. Dual axes show median \texttt{upset\_simple} and median removed weight within each panel. Cross-panel numerical comparisons are not intended; A5/A6 are excluded from this trajectory.}
\label{fig:ablation}
\end{figure}

Table~\ref{tab:reviewer1_stage_ablation} reports \texttt{upset\_simple}, \texttt{upset\_naive}, \texttt{upset\_ratio}, and mean single-invocation algorithm runtime across three pipeline stages, on matched dataset support.
The legacy fixed-topological/\textsc{ins} reinsertion (A1) and its refined full pipeline (A3) were run on a smaller $n=33$ scope than the canonical reachability stages ($n=77$); comparing them against a Phase~A row computed on the larger scope would confound the stage effect with a change in dataset composition.
We therefore report two panels, each internally on one exact common-completion dataset set: Panel~(a) compares Phase~A only, legacy reinsertion, and the legacy refined pipeline on their shared $n=33$ scope; Panel~(b) compares Phase~A only, exact reachability reinsertion, and refinement (\textsc{OURS-Reach}) on their shared $n=77$ scope.
Within each panel, mean \texttt{upset\_simple}, \texttt{upset\_naive}, and \texttt{upset\_ratio} decrease monotonically from Phase~A only to the full pipeline, and mean algorithm runtime increases monotonically with pipeline depth; the significant paired improvement of exact reachability add-back over Phase~A only and over the legacy proxy is already quantified in Table~\ref{tab:ablation_primary}, which these per-panel means complement rather than replace.
Runtime is reported as \texttt{runtime\_algorithm\_sec} (Phase~A + Phase~B + Phase~C of one \textsc{OURS-Reach} call, none of the configurations in this table use min-cut), not the harness-level \texttt{runtime\_total\_sec} recorded per run: for every stage past Phase~A only, the per-run harness timer also executes a diagnostic Phase-A-only rerun (used solely to compute a permutation-distance sensitivity statistic, \S\ref{subsec:sensitivity}) inside the same timed window, so \texttt{runtime\_total\_sec} is inflated by roughly one extra Phase-A cost and is not the wall time of a single invocation.

\begin{table}[H]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\textbf{Panel (a): legacy fixed-topological/\textsc{ins} progression --- common $n=33$}\\[2pt]
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Stage} & \texttt{upset\_simple} & \texttt{upset\_naive} & \texttt{upset\_ratio} & Mean algorithm runtime (s) \\
\midrule
Phase~A only (A0) & 0.2571 & 93621.2 & 0.3762 & 0.122 \\
+ legacy fixed-topological/\textsc{ins} reinsertion (A1) & 0.2555 & 93589.0 & 0.3760 & 0.127 \\
+ legacy refinement, full pipeline (A3) & 0.2521 & 93548.8 & 0.3462 & 0.227 \\
\bottomrule
\end{tabular}

\vspace{8pt}
\textbf{Panel (b): canonical reachability progression --- common $n=77$}\\[2pt]
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Stage} & \texttt{upset\_simple} & \texttt{upset\_naive} & \texttt{upset\_ratio} & Mean algorithm runtime (s) \\
\midrule
...
+ refinement, \textsc{OURS-Reach} (A4) & 0.2724 & 118722.0 & 0.3178 & 0.364 \\
\bottomrule
\end{tabular}
\caption{Stage-wise ablation on matched dataset support. Panel~(a) reports the legacy fixed-topological/\textsc{ins} progression on its common-completion subset ($n=33$); Panel~(b) reports the canonical reachability-based progression on its common-completion subset ($n=77$). Entries are means over the indicated support; mean single-invocation algorithm runtime (\texttt{runtime\_algorithm\_sec} = Phase~A + Phase~B + Phase~C) is reported alongside the three upset losses (primary runtime comparisons elsewhere use the median, as in Table~\ref{tab:runtime_wtl}). The $33$ datasets in Panel~(a) are a subset of those in Panel~(b), so cross-panel numerical comparisons are not intended; paired significance is reported in Table~\ref{tab:ablation_primary}.}
...
The optional min-cut exchange is a \emph{secondary} structural contribution: it is not part of \textsc{OURS-Reach} (Tables~\ref{tab:pairwise_quality_simple}--\ref{tab:runtime_wtl}).
In the ablation harness, A5 applies min-cut after reachability without Phase~C; A6 runs Phase~C refinement before the exchange and then extracts post-exchange topological scores for upset reporting (so A6 upset deltas are not ``A4 scores plus min-cut'').
Phase~C does not modify the retained arc set, so the A2$\to$A5 and A4$\to$A6 \emph{removed-weight} improvements in Table~\ref{tab:ablation_primary} remain valid structural evidence for Proposition~\ref{prop:mincut}.
...
We therefore claim monotone weighted-FAS improvement where exchanges are accepted, not uniform ranking dominance from min-cut.
...
Family-aware analysis shows that SpringRank comparisons remain family-sensitive, and BTL remains stronger on \texttt{upset\_ratio} under equal-family scrutiny (\S\ref{subsec:family_aware}). Corrected RankCentrality is stronger than \textsc{OURS-Reach} on pairwise \texttt{upset\_ratio} (Table~\ref{tab:pairwise_quality_ratio}); family-aware RankCentrality macros were not recomputed after the baseline correction and are not used as evidence.
...
We addressed ranking from weighted pairwise comparisons with a training-free structural pipeline: local-ratio cycle breaking, exact reachability-aware reinsertion, optional weighted min-cut exchange, and score extraction with optional Phase~C refinement (adjacent-swap then order-preserving ternary).
...
On common completions, \textsc{OURS-Reach} is strong on simple/naive upset metrics against the listed classical and GNN baselines, remains weaker than BTL and corrected RankCentrality on upset\_ratio, and is family-sensitive versus SpringRank despite a significant pairwise Holm test on upset\_simple.
It is slower than most lightweight classical methods (comparable to SyncRank and corrected RankCentrality), substantially faster than the evaluated trained GNN pipelines under the archived end-to-end protocol, and encounters a large-dense limit on Finance.
...
The datasets used in the experiments originate from prior benchmarks cited in the manuscript (e.g., \cite{he22}). For reproducibility, the processed suite graphs used in this study are provided in the public repository linked below, primarily as sparse adjacency matrices in \texttt{.npz} form (with some families also retaining text edge-list sources). Two intended suite members lack loadable adjacency files in the archived release (\texttt{ERO/p5K5N350eta10styleuniform}, \texttt{Halo2BetaData/HeadToHead}), so loadable analyses use the $78$-graph denominator stated in \S\ref{subsec:exp_setup}.
