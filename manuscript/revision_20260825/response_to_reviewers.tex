...
I have consolidated the existing ablation results into Table~8 so that the three pipeline stages requested by the reviewer -- Phase~A only (A0), Phase~A with reinsertion (A2, the revised canonical exact-reachability reinsertion), and the full refined pipeline (A4, \textsc{OURS-Reach}) -- are directly compared using all three requested upset measures (\texttt{upset\_simple}, \texttt{upset\_naive}, \texttt{upset\_ratio}) and mean runtime in one place. A0/A2/A4 share the identical $n=77$ non-Finance common-completion set. Table~8 also reports one additional row for the fixed-topological/INS reinsertion proxy used in the \emph{submitted} manuscript (A1, $n=33$ on its available Layer-1 core scope), so the reviewer's original wording ("Phase~1 with reinsertion") is answered under both the submitted and the revised reinsertion mechanism; A1 is not part of the revised canonical progression. These are consolidated/re-aggregated from the already-completed structural-ablation runs underlying Table~7 (no new scientific experiment was run). Directional significance (exact reachability add-back materially improving \texttt{upset\_simple} over Phase~A only, W/T/L $76/0/1$, $n=77$, Holm $p=2.4\times 10^{-12}$, and over the submitted topological proxy, $32/0/1$, $n=33$) is established by the paired tests in Table~7, which Table~8's per-stage means complement rather than replace. Legacy multipass topo-proxy passes P2/P3 add little beyond P1; \texttt{zero\_tol} is stable; refinement saturates quickly (R1$\approx$R2). Non-Finance A4 runtimes are roughly $0.01$--$1.2$\,s (median $\approx 0.57$\,s) for $n\le 602$; Finance is the large-dense boundary (Section~3.5).
