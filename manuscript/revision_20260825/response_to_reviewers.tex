...
\noindent\textbf{Response.}
We agree that the submitted fixed-topological proxy was too conservative and that multipass INS2/INS3 were not major quality innovations. Exact reachability restores the intended cycle-safety test and materially improves rankings (A0$\to$A2: $76/0/1$; A1$\to$A2: $32/0/1$). INS multipass is retained only historically. As a secondary mechanism we added a weighted min-cut exchange with monotone weighted-FAS improvement (Proposition~3; Section~3.9).
...
\noindent\textbf{Response.}
We revised claims to be metric-specific: strong simple/naive upset performance against several baselines; competitive (not uniformly superior) with SpringRank/DavidScore/SVD\_NRS under family-aware analysis; BTL remains stronger on \texttt{upset\_ratio}. We do not claim universal dominance. Together with Comments~1--3, the three concrete requests---reconcile tables, fix/strengthen insertion, and add classical runtime comparisons---are implemented.
...
\noindent\textbf{Response.}
We removed ``Scalable'' from the title and describe empirical behavior on evaluated sparse/moderately dense graphs, with Finance as an explicit large-dense boundary.
...
\noindent\textbf{Response.}
The A0--A6 structural ablation addresses backbone stages (Phase~A, topo proxy, reachability, refinement, min-cut). Cycle-selection sensitivity (C0/C1) shows a small but detectable effect on the A4 path ($|\mathrm{med}\,\Delta|\approx 10^{-3}$); we report it without overstatement.
