...
The \emph{canonical} revised method reported in the principal quality/runtime tables is \textsc{OURS-Reach}: Phase~A + exact reachability-aware reinsertion + Phase~C refinement (\S\ref{sec:framework}), \emph{without} the optional min-cut exchange.
...
Archived \textsc{OURS-MFAS}/\textsc{INS3} labels are duplicate submitted topo-proxy configurations retained only as historical/ablation contrasts.
...
Ablation CSVs document stage contributions and the topo-proxy vs.\ reachability contrast.
Stale submitted Table~4/5 aggregates are not reused.
...
Coverage on $79$ readable datasets: \textsc{OURS-Reach} $77/79$ ($97.5\%$; Finance timeout); most classical methods $78/79$ ($98.7\%$); SyncRank $77/79$; DIGRAC/ib $61/79$ ($77.2\%$, primarily not-applicable rather than timeout).
...
\caption{\textsc{OURS-Reach} (A4) wall time versus edge count on non-Finance graphs (log--log). Finance ($m\approx 1.73\times 10^{6}$) is marked as the large-dense boundary.}
...
Under the archived $1800$\,s non-GNN protocol, leaderboard topo-proxy OURS and SyncRank time out while lightweight classical methods finish in $<4$\,s and GNN end-to-end runs finish in $\sim 15$--$18$\,min.
