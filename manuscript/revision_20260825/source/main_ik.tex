...
Table~\ref{tab:reviewer1_stage_ablation} reports the three requested pipeline stages, with all three upset losses and mean runtime, on matched dataset support.
The legacy fixed-topological/\textsc{ins} reinsertion used in the submitted manuscript (A1) and its refined full pipeline (A3) were run on a smaller $n=33$ scope than the revised canonical stages; comparing them against a Phase~A row computed on the larger $77$-dataset scope would confound the stage effect with a change in dataset composition.
...
Within each panel, mean \texttt{upset\_simple}, \texttt{upset\_naive}, and \texttt{upset\_ratio} decrease monotonically from Phase~A only to the full pipeline, and mean runtime increases monotonically with pipeline depth; the significant paired improvement of exact reachability add-back over Phase~A only and over the submitted proxy is already quantified in Table~\ref{tab:ablation_primary}, which these per-panel means complement rather than replace.
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Stage} & \texttt{upset\_simple} & \texttt{upset\_naive} & \texttt{upset\_ratio} & Mean runtime (s) \\
\midrule
A0 (Phase~A only) & 0.2571 & 93621.2 & 0.3762 & 0.148 \\
A1 (+ legacy topo/\textsc{ins} add-back) & 0.2555 & 93589.0 & 0.3760 & 0.276 \\
A3 (+ legacy refinement, full pipeline) & 0.2521 & 93548.8 & 0.3462 & 0.378 \\
\bottomrule
\end{tabular}
...
\textbf{Stage} & \texttt{upset\_simple} & \texttt{upset\_naive} & \texttt{upset\_ratio} & Mean runtime (s) \\
\midrule
A0 (Phase~A only) & 0.2853 & 120842.9 & 0.3498 & 0.174 \\
A2 (+ exact reachability add-back) & 0.2728 & 118788.6 & 0.3410 & 0.427 \\
A4 (+ refinement, \textsc{OURS-Reach}) & 0.2724 & 118722.0 & 0.3178 & 0.542 \\
\bottomrule
\end{tabular}
\caption{Stage-wise ablation on matched dataset support. Panel~(a) reports the legacy fixed-topological/\textsc{ins} progression (submitted manuscript) on its exact common-completion subset ($n=33$); Panel~(b) reports the revised reachability-based progression on its exact common-completion subset ($n=77$). All entries are means over the panel's indicated common support (mean runtime specifically answers the reviewer's request for average runtime; primary runtime comparisons elsewhere use the median, as in Table~\ref{tab:runtime_wtl}). The two panels use different dataset scopes (Panel~(a)'s $33$ datasets are a subset of Panel~(b)'s $77$) and are not intended for direct cross-panel comparison; directional significance is established by the paired tests in Table~\ref{tab:ablation_primary}.}
