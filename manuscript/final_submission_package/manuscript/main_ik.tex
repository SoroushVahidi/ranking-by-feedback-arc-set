...
\begin{abstract}
Ranking from weighted pairwise comparisons asks for a global order that respects observed preferences as well as possible. We study a training-free, deterministic pipeline that builds an acyclic backbone on the comparison digraph using an MWFAS-inspired local-ratio heuristic, restores high-weight arcs under exact cycle-safe reinsertion, optionally applies a secondary weighted min-cut exchange, and extracts scores with optional order-preserving refinement. The local-ratio and exact add-back components follow prior lineage; this paper formalizes ranking--MWFAS optimum-value equivalence and practical guarantee boundaries, corrects a submitted topological-proxy weakening of add-back, and expands classical and GNN evaluation under timeout-aware common-completion analysis. Empirically, under the archived leaderboard configuration used for direct baseline tables, the pipeline is strong on simple/naive upset metrics against several baselines, competitive rather than uniformly superior to SpringRank, DavidScore, and SVD\_NRS under family-aware analysis, and weaker than BTL on upset\_ratio; structural ablations separately quantify the reachability correction. The method is slower than lightweight classical estimators yet substantially faster than archived trained GNNRank runs under an end-to-end protocol; the near-complete Finance graph marks a large-dense scalability boundary.
\end{abstract}
...
Restore exact reachability-aware reinsertion in place of the submitted topological-position proxy (\S\ref{subsec:framework_phaseB}); ablations quantify the effect (\S\ref{subsec:ablation}).
...
A later practical codebase weakened reinsertion to a fixed topological-position proxy; multipass INS variants partially compensate and are historical comparison points only.
...
\end{enumerate}
Legacy multipass topo-proxy variants (INS1--3) are retained only as historical comparison points (\S\ref{sec:related}).
...
Archived \textsc{OURS-MFAS} and \textsc{INS3} labels are duplicate topo-proxy configurations; multipass INS variants are historical only.
Direct quality and classical/GNN runtime tables below reuse the archived \textsc{OURS\_MFAS} leaderboard column for continuity with the submitted evaluation archive; the incremental effect of restoring exact reachability (and of optional min-cut/refinement) is reported in the structural ablation (\S\ref{subsec:ablation}), not by silently replacing that archive column.
...
summarize archived \textsc{OURS\_MFAS} (topo-proxy leaderboard column; see protocol)
against principal baselines on pairwise common completions.
...
\bottomrule
\end{tabular}
\caption{\texttt{upset\_simple} (lower better) on pairwise common completions for archived \textsc{OURS\_MFAS}.
...
\bottomrule
\end{tabular}
\caption{\texttt{upset\_ratio} (lower better) on the same common completions for archived \textsc{OURS\_MFAS}.
Oracle best-in-suite envelopes are not headline competitors.}
\label{tab:pairwise_quality_ratio}
\end{table}

On \texttt{upset\_simple}, archived OURS shows strong W/T/L margins against BTL,
against SyncRank, SerialRank, and RankCentrality, and against \textsc{ib},
with significant Holm-adjusted Wilcoxon advantages vs BTL, \textsc{DIGRAC}, and \textsc{ib}.
Against \textsc{SpringRank}, \textsc{DavidScore}, and \textsc{SVD\_NRS}, per-dataset wins slightly favor OURS,
but Holm-adjusted tests are not significant; family-aware analysis (\S\ref{subsec:family_aware})
treats these as competitive rather than uniformly superior.
On \texttt{upset\_naive}, the same directional pattern holds
(e.g., $41/0/36$ vs SpringRank; $73/0/4$ vs BTL; $60/0/0$ vs \textsc{ib}).
On \texttt{upset\_ratio}, BTL remains clearly stronger ($1/0/76$) and PageRank also wins more often ($14/0/63$);
OURS is strong against DavidScore, SVD\_NRS, and RankCentrality.
...
\bottomrule
\end{tabular}
\caption{Paired runtime W/T/L on common completions (archived \textsc{OURS\_MFAS} timings).
...
On non-Finance ablation graphs ($n\le 602$), A4 finishes in roughly $0.01$--$1.2$\,s (median $\approx 0.57$\,s); enabling min-cut (A6) adds about $3$--$4$\,s median.
...
Legacy multipass topo-proxy: P1 restores many edges; P2/P3 add little ranking quality.
Min-cut budgets: K20$\to$K50 modest; K50$\to$K100 nearly flat. Detailed grids are deferred to the supplement.
...
Relative to lightweight classical estimators the pipeline is slower (\S\ref{subsec:runtime_coverage}); GNN speedups refer to archived end-to-end training+evaluation under imperfect hardware comparability (\S\ref{subsec:exp_setup}).
Family-aware analysis shows that SpringRank/DavidScore/SVD\_NRS comparisons are cautious, and BTL remains stronger on \texttt{upset\_ratio} (\S\ref{subsec:family_aware}).
...
The algorithmic core follows Demetrescu--Finocchi and \citet{VK25}; the revised contributions are formal ranking--MWFAS clarification and guarantee boundaries, restoration of exact cycle-safe add-back after a topological-proxy weakening, a secondary monotone min-cut repair, and a broader timeout-aware empirical study.

On common completions, the archived leaderboard configuration used for direct baseline tables is strong on simple/naive upset metrics against several classical and GNN baselines, competitive under family-aware scrutiny with SpringRank, DavidScore, and SVD\_NRS, and weaker than BTL on upset\_ratio; ablations quantify the reachability correction separately.
It is slower than lightweight classical methods and substantially faster than the evaluated trained GNN pipelines under the archived end-to-end protocol, with Finance exposing a large-dense limit.
