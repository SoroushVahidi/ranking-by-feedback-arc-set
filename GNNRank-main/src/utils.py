...
def get_powers_sparse(A, hop=3, tau=0.1):
    ...
    A_powers.append(torch.sparse_coo_tensor(torch.LongTensor(
        np.vstack(adj0_new.nonzero())), torch.FloatTensor(np.array(adj0_new.data).flatten()), shaping))
    A_powers.append(torch.sparse_coo_tensor(torch.LongTensor(
        np.vstack(ind_power)), torch.FloatTensor(np.array(tmp[ind_power]).flatten()), shaping))
    if hop > 1:
        A_power = A.copy()
...
def scipy_sparse_to_torch_sparse(A):
    A = sp.csr_matrix(A)
    A.eliminate_zeros()
    indices = np.vstack(A.nonzero())  # stack into single 2D numpy array first
    return torch.sparse_coo_tensor(
        torch.LongTensor(indices),
        torch.FloatTensor(A.data.copy()),
        A.shape
    )
...
def print_performance_mean_std(dataset: str, results: np.array, compare_names_all: list = default_compare_names_all,
    ...
    Args:
        ...
        print_std: (bool, optional) Whether to print standard deviations or just mean. Default False.
    """
