...
# Default per-method timeout in seconds (30 minutes).
# GNN methods get a separate, longer timeout.
DEFAULT_METHOD_TIMEOUT = 1800   # 30 min for baselines
GNN_METHOD_TIMEOUT = 7200       # 2 hours for GNN training


GNN_variant_names = ['dist', 'innerproduct', 'proximal_dist', 'proximal_innerproduct', 'proximal_baseline']
NUM_GNN_VARIANTS = len(GNN_variant_names)
...
class Trainer(object):
    ...
    def train(self, model_name):
        ...
        #################################
        # runtime_full: stores runtime in seconds for each method variant / split
        if model_name not in ['DIGRAC', 'ib']:
            t_method_start = time.time()
            try:
                # Set timeout via signal alarm (Unix only)
                old_handler = signal.signal(signal.SIGALRM, _timeout_handler)
                signal.alarm(DEFAULT_METHOD_TIMEOUT)
                
                kendalltau_full, upset_full, runtime_full = self.non_nn(model_name)
                
                signal.alarm(0)  # cancel alarm
                signal.signal(signal.SIGALRM, old_handler)
            except TimeoutError:
                signal.alarm(0)
                print(f"TIMEOUT: {model_name} exceeded {DEFAULT_METHOD_TIMEOUT}s limit!")
                kendalltau_full = np.zeros([self.splits, 3, 2])
                kendalltau_full[:] = np.nan
                upset_full = np.zeros([self.splits, NUM_UPSET_CHOICES])
                upset_full[:] = np.nan
                runtime_full = np.full(self.splits, np.nan)
            ...
            runtime_full_latest = runtime_full.copy()
        else:
            ...
            runtime_full = np.zeros([NUM_GNN_VARIANTS, self.splits])
            runtime_full[:] = np.nan
            runtime_full_latest = runtime_full.copy()
            ...
            for split in range(self.splits):
                ...
                try:
                    old_handler = signal.signal(signal.SIGALRM, _timeout_handler)
                    signal.alarm(GNN_METHOD_TIMEOUT)
                    ...
                    if self.args.baseline == 'SpringRank':
                        ...
                        score = np.real(syncRank_angle(self.A))
                    ...
                    trainable_alpha=self.args.trainable_alpha, initial_score=score_torch, prob_dim=self.num_clusters, sigma=self.args.sigma).to(self.args.device)
                        ...
                        raise NameError('Please input the correct GNN model name from:\
                            DIGRAC, ib instead of {}!'.format(model_name))
                    ...
                    t_gnn_training_time = time.time() - t_gnn_start
                    ...
                    # Record runtime for all GNN variants (same training time)
                    for vi in range(NUM_GNN_VARIANTS):
                        runtime_full[vi, split] = t_gnn_training_time
                    ...
                    # Record runtime for latest as well
                    for vi in range(NUM_GNN_VARIANTS):
                        runtime_full_latest[vi, split] = t_gnn_training_time

                    logstr += '\nRuntime: {:.4f} seconds'.format(t_gnn_training_time)
                    print(logstr)
                    ...
                    signal.alarm(0)
                    print(f"TIMEOUT: {model_name} split {split} exceeded {GNN_METHOD_TIMEOUT}s limit!")
                    # Leave NaN for this split
                    for vi in range(NUM_GNN_VARIANTS):
                        runtime_full[vi, split] = np.nan
                        runtime_full_latest[vi, split] = np.nan
                except Exception as e:
                    signal.alarm(0)
                    print(f"ERROR: {model_name} split {split} failed with: {e}")
                    traceback.print_exc()
                    for vi in range(NUM_GNN_VARIANTS):
                        runtime_full[vi, split] = np.nan
                        runtime_full_latest[vi, split] = np.nan

        return kendalltau_full, kendalltau_full_latest, upset_full, upset_full_latest, runtime_full, runtime_full_latest
    ...
    def non_nn(self, model_name):
            ...
            upset_full = np.zeros([self.splits, NUM_UPSET_CHOICES])
            upset_full[:] = np.nan
            runtime_full = np.zeros(self.splits)
            runtime_full[:] = np.nan
            A = scipy_sparse_to_torch_sparse(self.A).to(self.args.device)
            ...
            for split in range(self.splits):
                ...
                if model_name == 'SpringRank':
                    ...
                    score = SVD_NRS(self.A)
                elif model_name == 'OURS_MFAS_INS1':
                    score, _ = ours_MFAS_INS1(self.A)
                elif model_name == 'OURS_MFAS_INS2':
                    score, _ = ours_MFAS_INS2(self.A)
                elif model_name == 'OURS_MFAS_INS3':
                ...
                if self.args.SavePred:
                    np.save(self.log_path + '/' + model_name + '_pred' + str(split), pred_label)
                    np.save(self.log_path + '/' + model_name + '_scores' + str(split), score)
                ...
                logstr += 'runtime:,{:.4f},seconds,'.format(t_elapsed)
                print(logstr)
        
                with open(self.log_path + '/' + model_name + '_log' + str(split) + '.csv', 'a') as file:
                    file.write(logstr)
                    file.write('\n')
            return kendalltau_full, upset_full, runtime_full

# train and grab results
if args.debug:
...
final_runtime = np.zeros([len(compare_names_all), args.num_trials*len(args.seeds)])
final_runtime[:] = np.nan
final_runtime_latest = final_runtime.copy()
...
for random_seed in args.seeds:
    ...
    for method_name in args.all_methods:
        kendalltau_full, kendalltau_full_latest, upset_full, upset_full_latest, runtime_full, runtime_full_latest = trainer.train(method_name)
        if method_name not in ['DIGRAC', 'ib']:
            ...
            final_runtime[current_ind, current_seed_ind: current_seed_ind + args.num_trials] = runtime_full
            final_runtime_latest[current_ind, current_seed_ind: current_seed_ind + args.num_trials] = runtime_full_latest
            current_ind += 1
        else:
            ...
            final_runtime[current_ind: current_ind+NUM_GNN_VARIANTS, current_seed_ind: current_seed_ind + args.num_trials] = runtime_full
            final_runtime_latest[current_ind: current_ind+NUM_GNN_VARIANTS, current_seed_ind: current_seed_ind + args.num_trials] = runtime_full_latest
...
for save_dir_name in ['kendalltau', 'upset', 'runtime']:
...
np.save(os.path.join(dir_name,'runtime',method_str,save_name), final_runtime)
np.save(os.path.join(dir_name,'runtime_latest',method_str,save_name), final_runtime_latest)


new_shape_upset = (args.num_trials*len(args.seeds), len(compare_names_all), NUM_UPSET_CHOICES)
new_shape_runtime = (args.num_trials*len(args.seeds), len(compare_names_all), 1)
metric_names = ['test kendall tau', 'test kendall p', 'val kendall tau', 'val kendall p', \
'all kendall tau', 'all kendall p'] + upset_choices + ['runtime_sec']
...
print_performance_mean_std(args.dataset+'_latest', np.concatenate((kendalltau_res_latest.swapaxes(0,1).reshape(new_shape_kendalltau),
                final_upset_latest.swapaxes(0,1).reshape(new_shape_upset),
                final_runtime_latest.swapaxes(0,1).reshape(new_shape_runtime)), axis=-1), 
...
print_performance_mean_std(args.dataset+'_best', np.concatenate((kendalltau_res.swapaxes(0,1).reshape(new_shape_kendalltau),
                final_upset.swapaxes(0,1).reshape(new_shape_upset),
                final_runtime.swapaxes(0,1).reshape(new_shape_runtime)), axis=-1), 
                compare_names_all, metric_names, False)